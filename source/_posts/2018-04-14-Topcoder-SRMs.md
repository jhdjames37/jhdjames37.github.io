...
+ 如果当前的攻击力不够：
  ...
  - 如果它的代价为 $2$，那么有两种情况：
    ...
    + 用 $2$ 的代价直接购买他。
    此时，我们先选第一种（因为它的代价小），并将二者差值加入优先队列，表示第二种情况。这个时候我们会发现后面的操作依然是合法的。（假设原来两个数为 $a, b$，优先队列中存在 $b - a$。在之后在处理权值为 $c$ 的时候，优先队列中存在 $c - (b - a) = c - b + a$，那么在下次选择的时候，相当于花 $3$ 的代价选 $a + c$）~~也许是对的吧。。。。~~
